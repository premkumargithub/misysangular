...
describe('Directive: miBomsDataGrid', function () {
  ...
  var element;
	...
	beforeEach(inject(function ($compile, $templateCache, $rootScope, $timeout) {
		...
		element = $compile(
			'<mi-boms-data-grid></mi-boms-data-grid>'
		)($rootScope.$new());
	...
	it('to have a read function which queries Boms.query with query params', inject(function(Boms) {
		var queryParams = {perpage: 25, page:3};
		spyOn(Boms, 'query').andCallThrough();
		
		element.isolateScope().read(queryParams);
		
		expect(Boms.query).toHaveBeenCalledWith(queryParams);
	}));
});
