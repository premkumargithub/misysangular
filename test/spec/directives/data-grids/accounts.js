...
describe('Directive: miAccountsDataGrid', function () {
  ...
  var element;
	...
	beforeEach(inject(function ($compile, $templateCache, $rootScope, $timeout) {
		...
		element = $compile(
			'<mi-accounts-data-grid></mi-accounts-data-grid>'
		)($rootScope.$new());
	...
	it('to have a read function which queries Accounts.query with query params', inject(function(Accounts) {
		var queryParams = {perpage: 25, page:3};
		spyOn(Accounts, 'query').andCallThrough();
		
		element.isolateScope().read(queryParams);
		
		expect(Accounts.query).toHaveBeenCalledWith(queryParams);
	}));
});
