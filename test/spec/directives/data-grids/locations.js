...
describe('Directive: miLocationsDataGrid', function () {
  ...
  var element;
	...
	beforeEach(inject(function ($compile, $templateCache, $rootScope, $timeout) {
		...
		element = $compile(
			'<mi-locations-data-grid></mi-locations-data-grid>'
		)($rootScope.$new());
	...
	it('to have a read function which queries Locations.query with query params', inject(function(Locations) {
		var queryParams = {perpage: 25, page:3};
		spyOn(Locations, 'query').andCallThrough();
		
		element.isolateScope().read(queryParams);
		
		expect(Locations.query).toHaveBeenCalledWith(queryParams);
	}));
});
